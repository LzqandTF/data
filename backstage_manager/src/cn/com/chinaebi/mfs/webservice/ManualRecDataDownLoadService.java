 ...
 *
 */
public class ManualRecDataDownLoadService extends HttpServlet {
	private static final long serialVersionUID = 5802435885748848623L;

	private static Log log = LogFactory.getLog(ManualRecDataDownLoadService.class);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if(StringUtils.isNotBlank(version) && StringUtils.isNotBlank(tranCode)){
				if(StringUtils.equals(tranCode, "ZH0012")){ //手工调账下载接口
					...
					if(StringUtils.isBlank(mer_code)){
						resMsg = "手工调账下载-参数mer_code不能为空";
					}else if(StringUtils.isBlank(start_date)){
						resMsg = "手工调账下载-参数start_date不能为空";
					}else if(StringUtils.isBlank(end_date)){
						resMsg = "手工调账下载-参数end_date不能为空";
			...
			if(resCode.equals("000")){
				...
				boolean file_flag = CreateManualRecFileService.createFileAndUpload(mer_code, start_date, end_date, date.substring(0, 8),date);
				if(file_flag){
					final String fileSavePath_local = PropertiesUtils.readProperties("sgtz_fileSavePath_local");
					...
					final String fileSavePath_ftp = PropertiesUtils.readProperties("sgtz_fileSavePath_ftp");
					String filePath = fileSavePath_local + "/" + mer_code  + "/" + date.substring(0, 8) + "/" + date +"_sgtz.xls";//文件路径组成    基础路径+商户号+日期
