...
public abstract class _BaseRootDAO {
	...
	protected final static Map<String,ThreadLocal<Session>> currentSessionMap = new HashMap<String, ThreadLocal<Session>>();
	...
	protected Session getSession(String configFile) {
		if (null != session && session.isOpen()) return session;
			...
			Session s = currentSessionMap.get(configFile).get();
			if (null == s || !s.isOpen()) {
				s = sessionFactory.openSession();
				currentSessionMap.get(configFile).set(s);
			}
			return s;
		}
		else {
			Session s = null;
			if(currentSessionMap.get(configFile) == null){
				s = null;
			}else
				s = currentSessionMap.get(configFile).get();
			
			if (null == s || !s.isOpen()) {
				s = getSessionFactory(configFile).openSession();
				if(currentSessionMap.get(configFile) == null){
					ThreadLocal<Session> tmp = new ThreadLocal<Session>();
					tmp.set(s);
					currentSessionMap.put(configFile, tmp);
				}
			}
			return s;
		}
	...
	public static void initialize () {
		cn.com.chinaebi.dz.object.dao._RootDAO.initialize(
			(String) null);
	}
	...
	public static void initialize (String configFileName) {
		cn.com.chinaebi.dz.object.dao._RootDAO.initialize(
			configFileName,
			cn.com.chinaebi.dz.object.dao._RootDAO.getNewConfiguration(
				null));
	}
	...
	public static void initialize (String configFileName, Configuration configuration) {
		if (null != sessionFactoryMap && null != sessionFactoryMap.get(configFileName)) return;
			...
			if (null == configFileName) {
				...
				cn.com.chinaebi.dz.object.dao._RootDAO.setSessionFactory(
					null,
	...
	*/
	public SessionFactory getSessionFactory(String configFileName) {
		...
		if (null == sf){
			initialize(configFileName);
			sf = (SessionFactory) sessionFactoryMap.get(configFileName);
			if(null == sf)
				throw new RuntimeException("The session factory for '" + configFileName + "' has not been initialized (or an error occured during initialization)");
			return sf;
		}
	 ...
	 */
	public static void closeCurrentSession (String configFile) {
		Session s = currentSessionMap.get(configFile).get();
		if (null != s) {
			if (s.isOpen()) s.close();
			currentSessionMap.get(configFile).set(null);
		}
	}
	...
	public String getConfigurationFileName () {
		return "data_manager_hibernate.xml";
	}
