package cn.com.chinaebi.ryt.jtyh.dz.deal;
 ...
 */
public class RytJtyhBankTradeDz implements TradeDzHandler {
				...
				if(duizhangList != null && duizhangList.size() > 0){
					...
					if(objMap != null && tseqMap != null && tseqMap.keySet().size() > 0){
						...
						for (Object object : duizhangList) {
							...
							//对账数据匹配原始交易数据
							//Object[] oriObj = hlogDAO.queryOriginalData(reqSysStance, reqSysStance, originalTableName,instId, DataStatus.flag_2);
							if(oriObj != null){
								/*
						...
						log.info(stringPingJie.getStringPingJie("开始汇总银行网关",bankInst.getBankName(),"对账之后数据汇总"));
						...
						log.info(stringPingJie.getStringPingJie("银行网关",bankInst.getBankName(),"对账之后汇总条数",count));
						log.info(stringPingJie.getStringPingJie("结束汇总银行网关",bankInst.getBankName(),"对账之后数据汇总"));
						
						log.info(stringPingJie.getStringPingJie("核查银行网关",bankInst.getBankName(),"是否存在未对账数据-进入差错"));
						...
						if(duizhangNotDzList != null && duizhangNotDzList.size() > 0){
							...
							log.info(stringPingJie.getStringPingJie("核查银行网关",bankInst.getBankName(),"没有存在未对账数据-无需进入差错"));
						}
					}else{
						log.warn(stringPingJie.getStringPingJie("获取银行网关:",bankName,"开始日期:",startDate,",结束日期:",endDate,",没有原始交易数据"));
					}
				}else{
					log.warn(stringPingJie.getStringPingJie("银行网关",bankInst.getBankName()," 日期 ：",deductStmlDate," 未查询到未对账对账单数据"));
				}
			}else{
