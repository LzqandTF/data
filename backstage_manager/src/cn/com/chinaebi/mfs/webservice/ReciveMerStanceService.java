...
import cn.com.chinaebi.dz.service.MerFundStanceService;
 ...
 *
 */
public class ReciveMerStanceService extends HttpServlet {
	private static final long serialVersionUID = 4449057292492202622L;
	...
	protected void service(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			if(StringUtils.isNotBlank(version) && StringUtils.isNotBlank(tranCode)){
				MerFundStanceService merFundStanceService = MerFundStanceService.getInstance();
				if(StringUtils.equals(tranCode, "ZH0005")){ //商户流水查询
					...
					if(StringUtils.isBlank(mer_code)){
						resMsg = "商户流水查询-参数mer_code不能为空";
					}else if(StringUtils.isBlank(start_date)){
						resMsg = "商户流水查询-参数start_date不能为空";
					}else if(StringUtils.isBlank(end_date)){
						resMsg = "商户流水查询-参数end_date不能为空";
					}else if(StringUtils.isBlank(pageNo)){
						resMsg = "商户流水查询-参数pageNo不能为空";
					}else if(StringUtils.isBlank(pageNum)) {
						resMsg = "商户流水查询-参数pageNum不能为空";
					}else{
						Object object = merFundStanceService.queryCountAndMoney(mer_code, start_date, end_date);
						if (object != null) {
							Object[] obj = (Object[]) object;
							data_count = Integer.valueOf(obj[0].toString()); // 总数据条数
							amt_count = obj[1] == null ? 0d : Double.valueOf(obj[1].toString()); // 总金额
						}
						// 明细
						itmes = merFundStanceService.queryMerFundStanceByMerCode(mer_code, start_date, end_date, (Integer.valueOf(pageNo) - 1) * Integer.valueOf(pageNum), Integer.valueOf(pageNum));
						resCode = "000";
						resMsg = "";
					}
