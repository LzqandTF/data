...
import cn.com.chinaebi.dz.object.base.BaseRiqieSzghLstDAO;
import cn.com.chinaebi.dz.util.DYDataUtil;


public class RiqieSzghLstDAO extends BaseRiqieSzghLstDAO implements cn.com.chinaebi.dz.object.dao.iface.RiqieSzghLstDAO {
	
	private Log log = LogFactory.getLog(getClass());
	public RiqieSzghLstDAO () {}
	
	public RiqieSzghLstDAO (Session session) {
		super(session);
	}
	...
	public List<RiqieSzghLst> findRiqieSzghLst(String deductStlmDate) {
		...
		List<RiqieSzghLst> list = null; 
		if(deductStlmDate == null){
			log.error("RiqieSzghLstDAO findRiqieSzghLst deductStlmDate param is not null");
			return null;
		}
		try {
			...
			log.info("深圳工行对账查询日切原始交易开始时间 ："+startTime);
			log.info("深圳工行对账查询日切原始交易开始时间 ："+endTime);
			
			session = this.getSession();
			Query query = session.createQuery("from RiqieSzghLst where DeductStlmDate BETWEEN ? and ? and WhetherValid = 1 and WhtherInnerDz = 0 order by ReqTime,TrademsgType asc");
			...
			if(listResult != null && listResult.size() > 0){
				...
				log.warn("from RiqieSzghLst where DeductStlmDate BETWEEN ? and ? and WhetherValid = 1 and WhtherInnerDz = 0 order by ReqTime,TrademsgType asc is not data");
			}
		}catch (Exception e) {
			log.error(e);
		} finally {
	...
	public boolean updateClean(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_roll_bk = ? and deduct_stlm_date BETWEEN ? and ?");
	...
	public boolean updateClean(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and deduct_roll_bk = ? and deduct_sys_time BETWEEN ? and ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setString(2, reqSysStance);
...
//				}
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherTk = ?,zf_file_fee = ?,zf_fee = ?,zf_fee_bj = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_roll_bk = ? and deduct_stlm_date BETWEEN ? and ?");
	...
	@Override
	public boolean saveRiqieSzghLst(RiqieSzghLst riqieSzghLst,
			Integer flagStatus) {
		Session session = null;
		Transaction transaction = null;
		boolean flag = false; 
		try {
			session = this.getSession();
			transaction = session.beginTransaction();
			SQLQuery sqlQuery = session.createSQLQuery("select count(*) from riqie_szgh_lst where trade_id = ?");
			sqlQuery.setString(0, riqieSzghLst.getId());
	...
	public boolean updateNoNeedHandle(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_sys_time BETWEEN ? and ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setString(2, reqSysStance);
				sqlQuery.setInteger(3, tradeMsgType);
	...
	public boolean updateCleanAndError(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherQs = ?,bk_chk = ?,whetherErroeHandle = ? where deduct_sys_stance = ? and deduct_stlm_date BETWEEN ? and ? and deduct_roll_bk = ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
	...
	public boolean updateCleanAndError(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szgh_lst set whetherQs = ?,bk_chk = ?,whetherErroeHandle = ? where deduct_sys_stance = ? and deduct_stlm_date BETWEEN ? and ? and deduct_roll_bk = ? and trademsg_type = ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setInteger(2, whetherError);
				sqlQuery.setString(3, reqSysStance);
