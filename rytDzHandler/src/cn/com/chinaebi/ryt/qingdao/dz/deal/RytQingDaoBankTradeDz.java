package cn.com.chinaebi.ryt.qingdao.dz.deal;
 ...
 */
public class RytQingDaoBankTradeDz implements TradeDzHandler {
				...
				if(duizhangList != null && duizhangList.size() > 0){
					...
					if(objMap != null && tseqMap != null && tseqMap.keySet().size() > 0){
						...
						for (Object object : duizhangList) {
							...
							//对账数据匹配原始交易数据
							//Object[] oriObj = hlogDAO.queryOriginalData(reqSysStance, orderId, originalTableName,instId, DataStatus.flag_1);
							if(oriObj != null){
								/*
