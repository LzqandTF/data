...
import cn.com.chinaebi.dz.object.base.BaseRiqieSzzhLstDAO;
import cn.com.chinaebi.dz.util.DYDataUtil;


public class RiqieSzzhLstDAO extends BaseRiqieSzzhLstDAO implements cn.com.chinaebi.dz.object.dao.iface.RiqieSzzhLstDAO {
	private Log log = LogFactory.getLog(getClass());
	public RiqieSzzhLstDAO () {}
	
	public RiqieSzzhLstDAO (Session session) {
		super(session);
	}
	...
	public List<RiqieSzzhLst> findRiqieSzzhLst(String deductStlmDate) {
		...
		List<RiqieSzzhLst> list = null; 
		if(deductStlmDate == null){
			log.error("RiqieSzzhLstDAO findRiqieSzzhLst deductStlmDate param is not null");
			return null;
		}
		try {
			...
			log.info("深圳中行对账查询日切原始交易开始时间 ："+startTime);
			log.info("深圳中行对账查询日切原始交易开始时间 ："+endTime);
			
			session = this.getSession();
			Query query = session.createQuery("from RiqieSzzhLst where DeductStlmDate BETWEEN ? and ? and WhetherValid = 1 and WhtherInnerDz = 0 order by ReqTime,TrademsgType asc");
			...
			if(listResult != null && listResult.size() > 0){
				...
				log.warn("from RiqieSzzhLst where DeductStlmDate BETWEEN ? and ? and WhetherValid = 1 and WhtherInnerDz = 0 order by ReqTime,TrademsgType asc is not data");
			}
		}catch (Exception e) {
			log.error(e);
		} finally {
	...
	public boolean updateClean(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_roll_bk = ? and deduct_stlm_date BETWEEN ? and ?");
	...
	public boolean updateClean(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and deduct_roll_bk = ? and deduct_sys_time BETWEEN ? and ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setString(2, reqSysStance);
	...
	public boolean updateNoNeedHandle(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherQs = ?,bk_chk = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_sys_time BETWEEN ? and ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setString(2, reqSysStance);
				sqlQuery.setInteger(3, tradeMsgType);
...
//				}
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherTk = ?,zf_file_fee = ?,zf_fee = ?,zf_fee_bj = ? where deduct_sys_stance = ? and trademsg_type = ? and deduct_roll_bk = ? and deduct_stlm_date BETWEEN ? and ?");
	...
	public boolean updateCleanAndError(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherQs = ?,bk_chk = ?,whetherErroeHandle = ? where deduct_sys_stance = ? and deduct_stlm_date BETWEEN ? and ? and deduct_roll_bk = ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
	...
	public boolean updateCleanAndError(String reqSysStance, boolean clean,
		...
		if(StringUtils.isNotBlank(reqSysStance)){
			try {
				...
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery sqlQuery = session.createSQLQuery("update riqie_szzh_lst set whetherQs = ?,bk_chk = ?,whetherErroeHandle = ? where deduct_sys_stance = ? and deduct_stlm_date BETWEEN ? and ? and deduct_roll_bk = ? and trademsg_type = ?");
				sqlQuery.setBoolean(0, clean);
				sqlQuery.setInteger(1, bkchk);
				sqlQuery.setInteger(2, whetherError);
				sqlQuery.setString(3, reqSysStance);
	...
	@Override
	public boolean saveRiqieDljhLst(RiqieSzzhLst riqieSzzhLst,
			Integer flagStatus) {
		Session session = null;
		Transaction transaction = null;
		boolean flag = false; 
		try {
			session = this.getSession();
			transaction = session.beginTransaction();
			SQLQuery sqlQuery = session.createSQLQuery("select count(*) from riqie_szzh_lst where trade_id = ?");
			sqlQuery.setString(0, riqieSzzhLst.getId());
