...
import cn.com.chinaebi.dz.object.base.BaseOriginalShengjingbankLstDAO;
import cn.com.chinaebi.dz.util.DYDataUtil;

import java.sql.CallableStatement;


public class OriginalShengjingbankLstDAO extends BaseOriginalShengjingbankLstDAO implements cn.com.chinaebi.dz.object.dao.iface.OriginalShengjingbankLstDAO {
	
	private Log log = LogFactory.getLog(getClass());
	public OriginalShengjingbankLstDAO () {}
	
	public OriginalShengjingbankLstDAO (Session session) {
		super(session);
	}

	public List<OriginalShengjingbankLst> querySJYHTradeLstClearInnerData(String date,String status) {
		Session  session = null;
		List<OriginalShengjingbankLst> list = null;
		try{
			session = this.getSession();
			Date startTime = DYDataUtil.getformatConversionDate3(date);
			Date endTime = DYDataUtil.getformatConversionDate4(date);
			Query query = session.createQuery("from OriginalShengjingbankLst where TradeTime BETWEEN ? and ? and TradeResult = ?  order by TradeTime asc");
			...
			if(listResult != null && listResult.size() > 0){
				...
				log.warn("from OriginalShengjingbankLst where TradeTime BETWEEN '"+startTime+"' and '"+endTime+"' and TradeResult =  '"+status+"' is not data");
			}
		}catch(Exception e){
			log.error(e);
		}finally{
	...
	public boolean deleteOriginalShengjingbankLst(String tradeTime){
		...
		if(StringUtils.isNotBlank(tradeTime)){
			try{
				session = this.getSession();
				transaction = session.beginTransaction();
				SQLQuery query = session.createSQLQuery("select count(*) from original_shengjingbank_lst where trade_time like ?");
				query.setString(0, tradeTime + "%");
				Integer c = Integer.valueOf(query.uniqueResult().toString());
				if(c > 0){
					SQLQuery delete = session.createSQLQuery("delete from original_shengjingbank_lst where trade_time like ?");
	...
	public boolean getSJYHTradeLstData(String tradeTime) {
		...
		try {
			...
			if (session  != null) {
				...
				CallableStatement cs = (CallableStatement) session.connection().prepareCall("call proce_original_shengjingbank_lst(?)");
				cs.setObject(1, tradeTime);
				cs.execute();
				ts.commit();
				flag = true;
			} else 
				log.info("getSJYHTradeLstData 获取session 为null");
		} catch (Exception e) {
			ts.rollback();
			log.error("盛京银行交易数据汇总出错：" + e.getMessage());
		} finally {
			//关闭session
			if (session != null) {
				session.close();
	...
	public int getTotalCountOfDzSucessDataOfSjyh(String date,List<String> codeArr,int generateNumber){
		...
		try{
			...
			sql.append("select count(*) from original_shengjingbank_lst where trade_time BETWEEN ? and ? and trade_result = ? ");
			if(generateNumber == 2){
