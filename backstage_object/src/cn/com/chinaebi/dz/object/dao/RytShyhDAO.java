...
import cn.com.chinaebi.dz.object.base.BaseRytShyhDAO;


public class RytShyhDAO extends BaseRytShyhDAO implements cn.com.chinaebi.dz.object.dao.iface.RytShyhDAO {

	private Log log = LogFactory.getLog(getClass());
	public RytShyhDAO () {}
	
	public RytShyhDAO (Session session) {
		super(session);
	}

	@Override
	public boolean updateRytShyh(String tseq, int bk_chk,
		...
		try {
			...
			SQLQuery query = session.createSQLQuery("update ryt_shyh set bk_chk = ?,whetherQs = ?,zf_fee = ?,zf_file_fee = ?,mer_fee = ?,zf_fee_bj = ?,deduct_stlm_date = ? where tseq = ?");
	...
	public Object[] selectTradeAmount(String tseq) {
		...
		try {
			session = this.getSession();
			SQLQuery sqlQuery = session.createSQLQuery("select sys_date,sys_time,tstat,tseq,card_no,amount,mer_fee,mid,gid,gate from ryt_shyh where tseq = ?");
	...
	@Override
	public List findRytShyhList(String sysDate,int gid,int bk_chk){
		Session session = null;
		List list = null;
		try {
			session = this.getSession();
			Integer date = Integer.valueOf(sysDate.replaceAll("-", ""));
			SQLQuery sqlQuery = session.createSQLQuery("select sys_date,sys_time,tstat,tseq,card_no,amount,mer_fee,mid,gid,gate,oid from ryt_shyh where sys_date = ? and gid = ? and bk_chk = ?");
	...
	@Override
	public List<RytShyh> selectRytShyh(String sysDate,
			Integer tstat, Integer bk_chk) {
		List<RytShyh> list = null;
		Session session = null;
		try {
			Integer date = Integer.valueOf(sysDate.replaceAll("-", ""));
			session = this.getSession();
			Query query = session.createQuery("from RytShyh where SysDate = ? and Tstat = ? and BkChk = ?");
	...
	@Override
	public Integer selectRytShyhCount(String sysDate) {
		Session session = null;
		Integer count = 0;
		try {
			Integer date = Integer.valueOf(sysDate.replaceAll("-", ""));
			session = this.getSession();
			SQLQuery query = session.createSQLQuery("select count(*) from ryt_shyh where sys_date = ?");
			query.setInteger(0, date);
	...
	public boolean updateNoNeedHandle(String sysDate, Integer bk_chk,
		...
		try {
			...
			session = this.getSession();
			transaction = session.beginTransaction();
			SQLQuery query = session.createSQLQuery("update ryt_shyh set bk_chk = ?,whetherQs = ? where sys_date = ? and tstat = ?");
			query.setInteger(0, bk_chk);
			query.setBoolean(1, whetherQs);
