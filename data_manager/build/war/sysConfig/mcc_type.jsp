...
<script type="text/javascript">
		...
		function init(){
			$.ajax({
				...
				success : function(msg) {
					for (i in msg){
						$("#mcc_big_type").append('<option value="' + msg[i]['big_type_id'] + '">'+ msg[i]['type_name'] + '</option>');
						$("#mcc_big_type_update").append('<option value="' + msg[i]['big_type_id'] + '">'+ msg[i]['type_name'] + '</option>');
					}
				},
				error : function(msg) {
					alert("获取渠道列表失败!");
		...
		function paging(pageNo) {
			var form = document.getElementById("mccTypeSearch");
			with (form) {
				action = "<%=request.getContextPath()%>/queryMccTypePage.do?pageNum=" + pageNo;
				method = "post";
				form.submit();
			}
		}
		...
		function queryByPageSize(pageSize) {
			var form = document.getElementById("mccTypeSearch");
			with (form) {
				action = "<%=request.getContextPath()%>/queryMccTypePage.do?pageSize=" + pageSize;
				method = "post";
				form.submit();
			}
		}
		...
		function checkQuery(){	
			document.getElementById("mccTypeSearch").submit();
		}
		...
		function addHandlingSub(){
			...
			var mcc_big_type = $("#mcc_big_type").val();
			if(mcc_big_type == null || mcc_big_type == ""){
				alert("请选择Mcc大类！");
				return;
			}
			$.ajax({
				type : "post",
				url : "<%=request.getContextPath()%>/addMccType.do",
				data : "type_name="+ type_name+"&big_type_id="+mcc_big_type,
				dataType : "text",
				success : function(msg) {
					if (msg == '1') {
						alert("添加成功！");
		...
		function selectData(id,type_name,big_type_id){
			$("#id_update").val(id);
			$("#type_name_update").val(type_name);
			
			var mcc_big_type = document.getElementById("mcc_big_type_update");
			for(var i = 0;i<mcc_big_type.options.length;i++){
				if(mcc_big_type.options[i].value == big_type_id){
					mcc_big_type.options[i].selected = 'selected';
				}
			}
		...
		function updateData(){
			...
			var big_type_id = $("#mcc_big_type_update").val();
			...
			$.ajax({
				type : "post",
				url : "<%=request.getContextPath()%>/updateMccType.do",
				data : "id="+ id +"&type_name="+type_name+"&big_type_id="+big_type_id,
				dataType : "text",
				success : function(msg) {
					if(msg == '1'){
						alert("修改成功");
		...
		function deleteData(id,name){
			if(confirm("确定要删除"+name+"类型的Mcc数据吗?")){
				$.ajax({
					type : "post",
					url : "<%=request.getContextPath()%>/deleteMccType.do",
					data : "id="+ id,
					dataType : "text",
					success : function(msg) {
						if(msg == '1'){
...
<body onload="init();">
	<div class="content">
		<div class="right" style="margin: 25px 5px;">
			...
			<div class="check clearfix">
				<h1 class="tit">查询区</h1>
				<form action="<%=request.getContextPath()%>/queryMccTypePage.do" target="right" name="mccTypeSearch" id = "mccTypeSearch" method="post">
					<ul class="check-m">						
						<li>
							<b>Mcc类型名称</b>
			...
			<div class="table-m">
				<table width="100%" border="0" cellspacing="0">
					...
					<c:forEach items="${pageDataLst.result }" var="mccType">
						<tr>
							<td align="center">${mccType.id }</td>
							<td align="center">${mccType.type_name }</td>
							<td align="center" width="60">
								<a href="javascript:deleteData(${mccType.id},'${mccType.type_name }')">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:selectData(${mccType.id},'${mccType.type_name }',${mccType.big_type_id })">修改</a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<div id="insert" class="pop" style="display: none">
		<div class="pop_body">
			<h1 class="pop_tit">
				<span class="fl">Mcc细类添加</span> 
				<a class="close" href="javascript:hide('insert')">&nbsp;</a>
			</h1>
			<div class="table_2">
				<table width="100%" border="0" cellspacing="0" id="instInfo">	
					<tr>
						<td align="right" bgcolor="#eeeeee">Mcc大类：</td>
						<td>
							<span > 
								<select id="mcc_big_type" name="mcc_big_type" >
					...
					<tr>
						<td align="right" bgcolor="#eeeeee">Mcc细类名称：</td>
						<td>
							<span class="input_bgl"> 
			...
			<div class="table_2">
				<table width="100%" border="0" cellspacing="0" id="login">					
					...
					<tr>
						...
						<td>
							<span > 
								<select id="mcc_big_type_update" name="mcc_big_type_update" >
								</select> 
							</span>
							<font color='red' size="4" style="margin-left: 2px;float:left;">*</font>
						</td>
