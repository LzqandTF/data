...
public abstract class EncodeUtils {
    ...
    public static String encodeStringMD5(String input) {
        ...
        try {
            ...
            byte[] array = md.digest(input.getBytes(DEFAULT_URL_ENCODING));

            for (int i = 0; i < array.length; i++) {
                sb.append(Integer.toHexString((array[i] & 0xFF) | 0x100).toUpperCase().substring(1, 3));
            }
        } catch (NoSuchAlgorithmException e) {
            throw new IllegalStateException("MD5 NoSuchAlgorithmException", e);
        } catch (UnsupportedEncodingException e) {
            throw new IllegalStateException("MD5 UnsupportedEncodingException", e);
        }
        return sb.toString();
    }
    ...
    public static void main(String[] args) {
        String value = "libo<br>jacky";
        String evalue = htmlEscape(value);
        System.out.println(evalue);
        String dvalue = htmlUnescape(evalue);
        System.out.println(dvalue);
    }
    
    /**
	 * 将摘要信息转换成MD5编码（转码成16进制）
	 * 
	 * @param message
	 *            摘要信息
	 * @return MD5编码之后的字符串
