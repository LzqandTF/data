 ...
 */
public class RytYlShouJiZhiFuBankTradeDz implements TradeDzHandler {
			...
			if(bankInst != null){
				...
				if(duizhangList != null && duizhangList.size() > 0){
					...
					if(objMap != null && tseqMap != null && tseqMap.keySet().size() > 0){
						...
						Integer qs_deductStlmDate = Integer.valueOf(deductStmlDate);
						qs_deductStlmDate = qs_deductStlmDate - 1;
						log.info(stringPingJie.getStringPingJie("银行网关",bankInst.getBankName(),"日期 ",qs_deductStlmDate,"未对账已清算数据查询"));
						List<Object> sjzfs = hlogDAO.queryNoDzJsOriginalData(originalTableName,qs_deductStlmDate, DataStatus.not_dz,DataStatus.clean);
						if(sjzfs != null && sjzfs.size() > 0){
							...
							log.info(stringPingJie.getStringPingJie("银行网关",bankInst.getBankName(),"日期 ",qs_deductStlmDate,"日期无未对账已清算数据"));
						}
