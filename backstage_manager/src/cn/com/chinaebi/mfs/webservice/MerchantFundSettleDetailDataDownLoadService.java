 ...
 *
 */
public class MerchantFundSettleDetailDataDownLoadService extends HttpServlet {
	private static final long serialVersionUID = 5802435885748848623L;

	private static Log log = LogFactory.getLog(MerchantFundSettleDetailDataDownLoadService.class);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if(StringUtils.isNotBlank(version) && StringUtils.isNotBlank(tranCode)){
				if(StringUtils.equals(tranCode, "ZH0013")){ //结算单明细下载
					...
					if(StringUtils.isBlank(mer_code)){
						resMsg = "结算单明细下载-参数mer_code不能为空";
					}else if(StringUtils.isBlank(start_date)){
						resMsg = "结算单明细下载-参数start_date不能为空";
					}else if(StringUtils.isBlank(end_date)){
						resMsg = "结算单明细下载-参数end_date不能为空";
					}else if (StringUtils.isNotBlank(backUrl)) {
						resMsg = "手工调账下载-参数backUrl不能为空";
					}else{
						resCode = "000";
						resMsg = "";
					}
			...
			if(resCode.equals("000")){
				...
				boolean file_flag = CreateMerchantFundSettleDetailFileService.createFileAndUpload(mer_code, start_date, end_date, date.substring(0, 8),date);
				if(file_flag){
					final String fileSavePath_local = PropertiesUtils.readProperties("jsdmx_fileSavePath_local");
					...
					final String fileSavePath_ftp = PropertiesUtils.readProperties("jsdmx_fileSavePath_ftp");
					String filePath = fileSavePath_local + "/" + mer_code  + "/" + date.substring(0, 8) + "/" + date +"_jsdmx.xls";//文件路径组成    基础路径+商户号+日期
