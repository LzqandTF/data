 ...
 */
public class RytYlWapBankTradeDz implements TradeDzHandler {
	...
	public void onLineTradeDzDeal(Integer startDate,
		...
		try {
			...
			if(bankInst != null){
				...
				Map<String, Object[]> columnMap = hlogDAO.queryColumnMap(originalTableName, instId, startDate, endDate,"bk_seq1");
...
//							String orderId = dzObj[4] == null ? "" : dzObj[4].toString();
							...
							//先使用流水号匹配
							Object[] oriObj = columnMap.get(reqSysStance);
							//对账数据匹配原始交易数据 reqSysStance -> bk_seq1
							//Object[] oriObj = hlogDAO.queryOriginalData(reqSysStance, orderId, originalTableName,instId, DataStatus.flag_3);
							if(oriObj != null){
								/*
								...
								log.warn(stringPingJie.getStringPingJie("流水： ",reqSysStance," 未匹配到原笔交易数据"));
							}
						}
