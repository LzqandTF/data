DROP PROCEDURE
IF EXISTS proce_creditcardpay_trade_lst;
CREATE PROCEDURE proce_creditcardpay_trade_lst(in tradeTime VARCHAR(20))
BEGIN
		INSERT INTO creditcardpay_trade_lst(
		trade_id,
		terminal_id,
		terminal_info,
		terminal_type,
  ...
  t.whetherzero
		FROM
			trade_lst t
		WHERE
		  t.trade_time BETWEEN DATE_FORMAT(CONCAT(tradeTime,'00:00:00'),'%Y-%m-%d %T') and DATE_FORMAT(CONCAT(tradeTime,'23:59:59'),'%Y-%m-%d %T') AND (t.req_process = '190000' OR t.trademsg_type = 12);
end;