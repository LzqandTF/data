...
declare user_id Varchar(20);-- 电银账户号 账户系统userId 
   ...
   select a.bank_acct,a.bank_name,a.bank_acct_name,a.pbk_no,a.bank_branch,a.bank_acct_name,a.category,a.exp_date,a.gate_id,a.liq_limit,a.liq_obj,a.liq_period,a.liq_state,a.liq_type,a.mer_type,a.name,a.id,a.pbk_acc_name,a.pbk_acc_no,a.pbk_name,a.prov_id,a.abbrev,a.begin_date,a.mstate,a.man_liq,a.auto_df_state,a.user_id,a.open_bk_no from minfo a;

	 DECLARE EXIT HANDLER FOR NOT FOUND SET done = 1;
   OPEN otherDatacur;
		 otherData:
			  LOOP
					FETCH otherDatacur INTO bank_acct,bank_name,bank_acct_name,pbk_no,bank_branch,bank_acct_name,category,exp_date,gate_id,liq_limit,liq_obj,liq_period,liq_state,liq_type,mer_type,mer_name,id,pbk_acc_name,pbk_acc_no,pbk_name,prov_id,abbrev,begin_date,mstate,man_liq,is_auto_df,user_id,open_bk_no;		
          IF done = 1 THEN
						LEAVE otherData;
					END IF;
               ...
               START TRANSACTION;
                 ...
                 insert into mer_billing(id,bil_object,bil_bank,bil_bankname,bil_accountname,bil_bankaccount,bil_way,bil_smallamt,bil_cycle,bil_manual,bil_account,bil_type,mer_code,bil_status)
						     values (REPLACE(UUID(),'-',''),liq_obj,temp_bank_no,temp_bank_name,temp_acct_name,temp_acct,liq_type,liq_limit,liq_period,man_liq,user_id,bill_type,id,liq_state);  
               COMMIT;
			END LOOP otherData;
   CLOSE otherDatacur;
 END