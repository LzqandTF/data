...
BEGIN
    ...
    ELSEIF flag = 2 then 
			set receipt_column_name = "deduct_stlm_date";
		...
		-- 原始收款交易按日期统计
	 	set @total_receipt_count_Sql = concat("select count(*) into @total_receipt_count from ",receipttablename ," where ",receipt_column_name," = '",original_tradeTime,"' and gid = ",gid,";");
		...
		-- 原始收款交易按日期统计对账成功
		set @total_receipt_bkchk1_Sql = concat("select count(*) into @total_receipt_bkchk1 from ",receipttablename ," where ",receipt_column_name," = '",original_tradeTime,"' and gid = ",gid," and bk_chk = 1;");
		...
		-- 原始收款交易按日期统计对账失败
		set @total_receipt_bkchk2_Sql = concat("select count(*) into @total_receipt_bkchk2 from ",receipttablename ," where ",receipt_column_name," = '",original_tradeTime,"' and gid = ",gid," and bk_chk = 2;");
		...
		-- 原始收款交易按日期统计未对账
		set @total_receipt_bkchk0_Sql = concat("select count(*) into @total_receipt_bkchk0 from ",receipttablename ," where ",receipt_column_name," = '",original_tradeTime,"' and gid = ",gid," and bk_chk = 0;");
		...
		-- 原始收款交易按日期统计无需对账
		set @total_receipt_bkchk3_Sql = concat("select count(*) into @total_receipt_bkchk3 from ",receipttablename ," where ",receipt_column_name," = '",original_tradeTime,"' and gid = ",gid," and bk_chk = 3;");
		...
		-- 对账交易按日期交易数据统计
		set @dz_total_count_Sql1 = concat("select count(*) into @dz_total_count1 from ",duizhang_table_name ," where ",duizhang_column_name," like '",dz_tradeTime,
					"%';");
