...
import cn.com.chinaebi.dz.service.ChannelDzCollectService;
 ...
 *
 */
public class ReciveMerchantFundSettleDetailService extends HttpServlet {
	private static final long serialVersionUID = 2730562509049171666L;
	
	private static Log log = LogFactory.getLog(ReciveMerchantFundSettleDetailService.class);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			if(StringUtils.isNotBlank(version) && StringUtils.isNotBlank(tranCode)){
				ChannelDzCollectService channelDzCollectService = ChannelDzCollectService.getInstance();
				if(StringUtils.equals(tranCode, "ZH0007")){ //结算单明细查询
					...
					if(StringUtils.isBlank(mer_code)){
						resMsg = "结算单明细查询-参数mer_code不能为空";
					}else if(StringUtils.isBlank(start_date)){
						resMsg = "结算单明细查询-参数start_date不能为空";
					}else if(StringUtils.isBlank(end_date)){
						resMsg = "结算单明细查询-参数end_date不能为空";
					}else if(StringUtils.isBlank(pageNo)){
						resMsg = "结算单明细查询-参数pageNo不能为空";
					}else if(StringUtils.isBlank(pageNum)) {
						resMsg = "结算单明细查询-参数pageNum不能为空";
					}else{
						Object object = channelDzCollectService.queryCountAndMoney(mer_code, start_date, end_date);
						if (object != null) {
						...
						// 结算单明细
						itmes = channelDzCollectService.queryManualRecLst(mer_code, start_date, end_date, (Integer.valueOf(pageNo) - 1) * Integer.valueOf(pageNum), Integer.valueOf(pageNum));
						resCode = "000";
						resMsg = "";
					}
