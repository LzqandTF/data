...
<title>内部差错处理方式模块查询</title>
<link href="<%=request.getContextPath()%>/css/style.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="<%=request.getContextPath()%>/js/jquery-1.7.2.min.js"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/js/timeOut.js"></script>
<script type="text/javascript">
		function paging(pageNo) {
			var form = document.getElementById("innerErrorHandlingSearch");
			with (form) {
				action = "<%=request.getContextPath()%>/getInnerErrorHandlingLst.do?pageNum=" + pageNo;
				method = "post";
				form.submit();
			}
		}
		...
		function checkQuery(){	
			document.getElementById("innerErrorHandlingSearch").submit();
		}
		...
		function addHandlingSub(){
			...
			if(handling_name_insert == null || handling_name_insert == ""){
				alert("内部差错处理方式名称不能为空！");
				return;
			}
			$.ajax({
				type : "post",
				url : "<%=request.getContextPath()%>/addInnerErrorHandlingLst.do",
				data : "handling_name="+ handling_name_insert,
				dataType : "text",
		...
		function selectData(handling_id,handling_name){
			$("#id_update").val(handling_id);
		...
		function updateData(){
			...
			$.ajax({
				type : "post",
				url : "<%=request.getContextPath()%>/updateInnerErrorHandlingLst.do",
				data : "handling_id="+ id +"&handling_name="+handling_name,
				dataType : "text",
				success : function(msg) {
					if(msg == '1'){
						alert("修改成功");
		...
		function deleteData(id,name){
			if(confirm("确定要删除"+name+"内部差错处理方式信息数据吗?")){
				$.ajax({
					type : "post",
					url : "<%=request.getContextPath()%>/deleteInnerErrorHandlingLst.do",
					data : "id="+ id,
					dataType : "text",
					success : function(msg) {
						if(msg == '1'){
...
<body>
	<div class="content">
		<div class="right" style="margin: 25px 5px;">
			<div class="position">
				当前位置：<a href="javascript:void(0)">系统配置</a>&gt;<span>内部差错处理方式配置查询</span>
			</div>
			<div class="check clearfix">
				<h1 class="tit">查询区</h1>
				<form action="<%=request.getContextPath()%>/getInnerErrorHandlingLst.do" target="right" name="innerErrorHandlingSearch" id = "innerErrorHandlingSearch" method="post">
					<ul class="check-m">						
						<li>
							<b>处理方式编号</b>
							<span class="input_bgl">
								<input maxlength="8" type="text" name="handling_id" id="handling_id" value="${param.handling_id }" />
							</span>
						</li>
						<li>
							<b>处理方式中文名称</b>
							<span class="input_bgl">
								<input maxlength="8" type="text" name="handling_name" id = "handling_name" value="${param.handling_name }" />
							</span>
						</li>
						<li class="cb mt0">
				...
				<table width="100%" border="0" cellspacing="0">
					<thead>
						<tr>
							...
							<td align="center" width="50%">内部差错处理方式名称</td>
					...
					<c:forEach items="${pageDataLst.result }" var="innerErrorHandling">
						<tr>
							<td align="center">${innerErrorHandling.handling_id }</td>
							<td align="center">${innerErrorHandling.handling_name }</td>
							<td align="center" width="60">
								<a href="javascript:deleteData(${innerErrorHandling.handling_id},'${innerErrorHandling.handling_name }')">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:selectData(${innerErrorHandling.handling_id},'${innerErrorHandling.handling_name }')">修改</a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<div id="update" class="pop" style="display: none">
		<div class="pop_body">
			<h1 class="pop_tit">
				<span class="fl">内部差错处理方式信息修改</span> 
			...
			<div class="table_2">
				<table width="100%" border="0" cellspacing="0" id="login">					
					...
					<tr>
						<td width="140" align="right" bgcolor="#eeeeee">内部差错处理方式名称：</td>
						<td>
							<span class="input_bgl"> 
