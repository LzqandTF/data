...
BEGIN
  ...
  DECLARE channel_inner_dz_cursor  cursor for select * from channel_inner_dz_view;
	...
	set @create_view_sql = concat("create view channel_inner_dz_view as select trade_id,req_sys_stance,deduct_sys_stance,deduct_result,req_process,trademsg_type,deduct_roll_bk,deduct_roll_bk_result,original_trans_info,trade_amount 
from ",tableName," where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and whetherValid = 1 and trademsg_type in(18,20,26,28,58,80,82);");
	...
	open channel_inner_dz_cursor;
		channel_inner_dz_loop:LOOP
			FETCH channel_inner_dz_cursor into _trade_id,_req_sys_stance,_deduct_sys_stance,_deduct_result,_req_process,_trademsg_type,_deduct_roll_bk,_deduct_roll_bk_result,_original_trans_info,_trade_amount;
			-- 避免循环多次
			IF done = 1 THEN
			...
			if _deduct_roll_bk = 1 THEN
				...
				if _deduct_roll_bk_result = 0 and (_trademsg_type = 26 or _trademsg_type = 80)  THEN
						set @update_cz_sql = concat("update ",tableName," set bk_chk = 1,whetherQs = 1 where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and req_sys_stance = '",_req_sys_stance,"';");
			...
			ELSE
				...
				if _deduct_result = 0 and (_trademsg_type = 18 or _trademsg_type = 58) THEN 
						...
						set @update_cx_sql = concat("update ",tableName," set bk_chk = 1,whetherQs = 1 where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and deduct_sys_stance = '",_req_sys_stance,"';");
						PREPARE update_cx_sql from @update_cx_sql;
						EXECUTE update_cx_sql;

						set @update_cx_ori_sql = concat("update ",tableName," set bk_chk = 1,whetherQs = 1 where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and deduct_sys_stance = '",RIGHT(_original_trans_info,6),"';");
				...
				ELSEIF _deduct_result = 0 and _trademsg_type = 20 THEN
						...
						set @select_th_ori_sql = concat("select trade_amount into @ori_amount where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and deduct_sys_stance = '",_original_stance,"';");
	...
	-- 修改内部对账反交易为退款
	set @update_whetherTk = concat("update ",tableName," set whetherTk = 1 where ",dateColumn," BETWEEN DATE_FORMAT('",startTime,"','%Y-%m-%d %T') and DATE_FORMAT('",endTime,"','%Y-%m-%d %T') and trademsg_type in(18,20,26,58,80) and whetherQs = 1");
